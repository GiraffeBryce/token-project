...
contract Faucet {
  ...
  constructor(uint256 dripAmount) {
    owner = msg.sender;
    allowance = dripAmount;
    BryceToken = IERC20(0x91A738deA8b317C8Ba6ABDEF013Aa69C8a78100A);
  }

  function name() public view virtual returns (string memory) {
    return "BryceToken Faucet";
  }
  ...
  function available() public view returns (uint256) {
    return BryceToken.balanceOf(address(this));
  }
  ...
  function _dripTo(address recipient) internal {
    ...
    if (available() >= transferAmount) {
      BryceToken.transfer(recipient, transferAmount);
  ...
  function _dripToAmount(address recipient, uint256 amt) internal {
    ...
    if (available() >= amt) {
      BryceToken.transfer(recipient, amt);
